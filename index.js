...
function trimProperties(obj) {
  // ✨ implement
  // const newVals = Object.values(obj).map((val) => val.trim());
  // return newVals;
  // const newObj = {
  //   foo: obj.foo.trim(),
  //   bar: obj.bar.trim(),
...
function findLargestInteger(integers) {
  const largest = integers.reduce(
    (acc, { integer }) => Math.max(acc, integer),
    0
  );
  return largest;
}
...
class Seasons {
  ...
  next() {
    ...
    this.currentSeason++;
    if (result === this.season[3]) {
      this.currentSeason = 0;
    }
    return result;
    // const diffSeason = [];
    // if (this.season === "summer") {
    //   //diffSeason.push("fall");
    //   return this.season[1];
    // } else if (diffSeason[0] === "fall") {
    //   diffSeason.push("winter");
    // } else if (diffSeason === "winter") {
    //   diffSeason.push("spring");
    // } else {
    //   diffSeason.push("summer");
    // }
    // return diffSeason[0];
  }
}
...
class Car {
  ...
  refuel(gallons) {
    // ✨ implement
    const miles = gallons * this.mpg;
    if (gallons >= 20) {
      this.odometer = this.odometer + 600;
      this.tank = 20;
    } else {
